...
public class Vala.Genie.Parser : CodeVisitor {
	...
	Statement parse_for_statement_with_var_operator_and_range () throws ParseError {
		...
		if (accept (TokenType.TO)) {
			...
			var to_src_ref = get_src (to_begin);
			var left = new MemberAccess (null, id, to_src_ref);
			var right = parse_primary_expression ();

			condition = new BinaryExpression (BinaryOperator.LESS_THAN_OR_EQUAL, left, right, to_src_ref);
			iterator = new PostfixExpression (left, true, to_src_ref);
		} 
		else if (accept (TokenType.DOWNTO)) {
			var downto_begin = get_location ();
			var downto_src_ref = get_src (downto_begin);
			/* create expression for condition and decrementing iterator */
			var left = new MemberAccess (null, id, downto_src_ref);
			var right = parse_primary_expression ();

			condition = new BinaryExpression (BinaryOperator.GREATER_THAN_OR_EQUAL, left, right, downto_src_ref);
			iterator = new PostfixExpression (left, false, downto_src_ref);
		}
		else {
	...
	Statement parse_for_statement_with_in_operator_and_range() throws ParseError {
		...
		var to_src_ref = get_src (to_begins);
		if (accept (TokenType.TO)) {
			to_downto_dots = 1;
		}
		else if (accept (TokenType.DOWNTO)) {
		...
		if (to_downto_dots == 1) {
			...
			if (int_amount >= 2) {
				...
				var condition_if_increasing = new BinaryExpression (BinaryOperator.LESS_THAN_OR_EQUAL, variable, int_end_expr, to_src_ref);
				var condition_if_decreasing = new BinaryExpression (BinaryOperator.GREATER_THAN_OR_EQUAL, variable, int_end_expr, to_src_ref);
				condition = new ConditionalExpression (first_lesser_second, condition_if_increasing, condition_if_decreasing, to_src_ref);

				var iterator_if_increasing = new PostfixExpression (variable, true, to_src_ref);
				var iterator_if_decreasing = new PostfixExpression (variable, false, to_src_ref);
				iterator = new ConditionalExpression (first_lesser_second, iterator_if_increasing, iterator_if_decreasing, to_src_ref);
			}
		}
